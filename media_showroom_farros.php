...
<html lang="en">
	...
	<head>
		...
		<!--- NOTIF -->
		<!--link rel="stylesheet" type="text/css" href="sos_style.css"-->
				...
				<header class="navbar navbar-default navbar-static-top">
					...
					<div class="navbar-header">
						...
						<a class="pull-right menu-toggler visible-xs-block" id="menu-toggler" data-toggle="collapse" href=".navbar-collapse">
							...
							<i class="ti-user"></i>
							<?php 
								if ($_SESSION['leveluser']=='admin' || $_SESSION['leveluser']=='MNGR' || $_SESSION['leveluser']=='DRKSI' || $_SESSION['leveluser']=='user' || $_SESSION['leveluser']=='HRD' || $_SESSION['leveluser']=='mngr_bengkel' || ($_SESSION['leveluser']=='supervisor' and $_SESSION['username']=='sudi')){
							?>
					...
					<div class="navbar-collapse collapse">
						<ul class="nav navbar-right">
							...
							<li class="dropdown">
								<?php 
									if ($_SESSION['username']=='farros' ||  $_SESSION['leveluser']=='MNGR'){
								?>
							...
							<li class="dropdown">
								<?php 
									if ($_SESSION['leveluser']=='admin' or $_SESSION['leveluser']=='CCO' or $_SESSION['leveluser']=='HRD' or $_SESSION['leveluser']=='MNGR'){
								?>
							...
							<li class="dropdown">
								...
								<ul class="dropdown-menu dropdown-light dropdown-messages dropdown-large">
									<li>
										<span class="dropdown-header"> Pengajuan Diskon</span>
									</li>
									<li>
										<div class="drop-down-wrapper ps-container">
	...
	<script src="vendor/bootstrap-toggle/js/bootstrap-toggle.js"></script>
		...
		if(leveluser == 'admin' || leveluser == 'HRD' || leveluser == 'CCO' || leveluser == 'MNGR' || leveluser == 'mngr_bengkel' ||leveluser == 'user' || leveluser == 'DRKSI'){
			function cek_aktivitas(){
			    ...
			    $.ajax({
			        ...
			        success: function(total_pesan_pengecekan){
						if(leveluser == 'MNGR' || leveluser == 'DKRSI' || leveluser == 'mngr_bengkel'){
							...
							success: function(total_pesan_pengajuan){
								...
								$.ajax({
									...
									success: function(total_hasil_pengecekan){
										if(leveluser == 'CCO' || leveluser == 'HRD'){
										...
										if(total_semua_pesan != ''){
											$("title").html("(" + total_semua_pesan + ") " + title_sos);
										}else{
											$("title").html("(" + "0" + ") " + title_sos);
										}
			...
			/*	if (!("Notification" in window)) {
			  ...
			  //================== MENAMPILKAN NOTIFIKASI PESAN PENGAJUAN DISKON ==================
				...
				var waktu = setTimeout("cek_aktivitas()",3000);
		...
		/*	var x = 1;
			function cek(){
				var hb = "Honda Bintaro - Sistem Operasional Sales";
				var lvr = "<?php echo $_SESSION['leveluser']?>";
				var tot = "<?php echo $count; ?>";
				var pesan = "<?php echo $pesan_baru ?>";
			    ...
			    $.ajax({
					...
					//	if(lvr == 'DRKSI' || lvr == 'MNGR' || lvr == 'user' || lvr == 'admin'){
						$.ajax({
							method: "post",
							url: "ceknotif.php",
							cache: false,
							...
							//	if(ntf != '' || lvr != ''){
								if(ntf != '' && (lvr == 'DRKSI' || lvr == 'MNGR' || lvr == 'user' || lvr == 'admin')){
									$("#myAudio").attr("autoplay", "true");
									var str = ntf.toString();
									var trm = str.trim();
									var spl = trm.split("|");
									var i;
									
										var str2 = spl.toString();
										var trm2 = str2.trim();
										var spl2 = trm2.split(",");
									//	console.log(spl + "/");
									//	console.log(spl2);
									...
									for (i=0; i <= (spl.length-i-i) ;i++){
										var redirect_onclick = spl2[4];
									
										if(lvr == 'user'){
											var isi = "Dear " + user + ", Pengajuan Diskon dengan no pengajuan " + spl2[0] + " sudah diproses";
										}else if(lvr == 'MNGR'){
											lvr = 'Pak Burli';
											var isi = "Dear " + lvr + (", terdapat ") + "Pengajuan Diskon dari " + spl2[1] + " dengan no pengajuan " + spl2[0];
										}else if(lvr = 'DRKSI'){
											lvr = 'Pak Tin Kok Sin';
											var isi = "Dear " + lvr + ", Pengajuan Diskon dengan no pengajuan " + spl2[0] + " menunggu persetujuan";
										}
										console.log(isi);
		...
		//==============================================================================================================
		//======================================================= UNTUK SHOW HIDE LEASING DAN TENOR PADA PENGAJUAN DISKON =======================================================
		...
		function harga_otomatis($jenis){
				...
				if (warna == null || tahun_buat == ""){
					...
					$.ajax({
						url: 'modul/pengajuan_diskon/harga_otomatis.php',
		...
		//=============================================================================
		   // BUAT HITUNG REFUND ====================================================
			function hitung_refund(){
				...
				if (leasing != "" && tenor != ""){
					...
					$.ajax({
						url: 'modul/pengajuan_diskon/hitung_refund.php',
				...
				*/
				...
				//$(window).load(function() { $(".preload-wrapper").fadeOut("slow"); }) 
				//document.getElementById("preload-wrapper").show();
				
				/*
				$('#leasing').on('change',function isi_otomatis(){
					var harga_otr = $("#harga_otr").val();
					var harga_otr2 = harga_otr.replace(/\D/g, '');
					var leasing = $("#leasing").val();
					var tenor = $("#tenor").val();
			...
			//$(').load(function() { $(".preload-wrapper").fadeOut("slow"); })
			
			
			
			
            
            // BUAT HARGA OTOMATIS BERDASARKAN TIPE ====================================================aaaaaaaa
			/*
			$('#tipe_mobil').on('change',function isi_otomatis(){
                var tipe_mobil = $("#tipe_mobil").val();
                var warna = $("#warna").val();
                $.ajax({
                    url: 'modul/pengajuan_diskon/harga_otomatis.php',
                    data: 'tipe_mobil='+tipe_mobil+'&warna='+warna,
                }).success(function (data) {
                    var json = data,
                    obj = JSON.parse(json);
                    var a = (obj.harga);
            ...
            $('#warna').on('change',function isi_otomatis(){
                var tipe_mobil = $("#tipe_mobil").val();
                var warna = $("#warna").val();
                $.ajax({
                    url: 'modul/pengajuan_diskon/harga_otomatis.php',
			...
			*/
			...
			//========================================================================================
			//====== MENAMPILKAN TOTAL DISKON PADA PENGAJUAN DISKON ==================================
			function startCalc(){
		...
		<!-- BUAT PENGAJUAN DISKON -->
			...
			$('#tipe_mobil').on('change',function(){
				...
				$.ajax({
					method : "post",
					url : "modul/pengajuan_diskon/get_warna_pd2.php",
					data : {data_ajax : isipd},
					success : function(data){
						$('#warna').html(data);
					}
		...
		<!--script type='text/javascript'>
		//<![CDATA[
		// TADINYA BUAT LOADING PAGE | SIMPEN AJA SIAPA TAU KEPAKE LAIN WAKTU
		$(document.body).append('<div id="preload-wrapper"></div>');
		// Saat halaman aktif terpicu untuk berpindah/keluar menuju halaman lain...
		$(window).on("beforeunload", function() {
			// ... tampilkan tabir animasi dengan efek `.fadeIn()`
			$('#preloader_4').fadeIn(1000).delay(6000).fadeOut(1000);
		});
		//]]>
		</script-->
		
		<!-- KALAU MAU PAKAI JEKUERI ONLEN -->
