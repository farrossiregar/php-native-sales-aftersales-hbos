...
?>
					...
					if(count($_POST['ubah'])) {
                        ...
                        $data=mysql_query("select * from target_serviceadvisor where bulan='$_POST[bulan]' and nama_sa = '$_POST[nama_sa]' order by urutan asc ");
					    $sql = $data;
						$nomor = 0;
						...
						mysql_query("update target_serviceadvisor set fix_pembagi='$fix_pembagi', nama_item='$nama_item', target_unit='$target_unit', target_point='$target_point' where urutan='$urutan' and nama_sa='$nama_sa' and bulan ='$bulan'");
               ...
               <script language="JavaScript">
				...
				<div class="main-content" >
								...
								<div class="col-md-12">
									<!--h5 class="over-title margin-bottom-15">Keseluruhan <span class="text-bold">Data Sales</span></h5-->
										...
										<p class="progress-demo">
											<button type = "submit" class="btn btn-wide btn-primary ladda-button" data-style="expand-right" onclick=window.location.href='?module=service_master_target_service_target_sa&act=buat';>
												<span class="ladda-label"><i class="fa fa-plus"></i> Input Target Sa Perbulan</span>
											</button>	
										</p>
										<hr>
										<?php
                                    ...
                                    <?php
                                    $query = "SELECT DISTINCT bulan FROM target_serviceadvisor";
									...
									<select name="nama_sa" id="nama_sa">
									<option value='' selected disabled class="blink">&#9654; PILIH SERVICE ADVISOR &#9664;</option>
                                    <?php
                                    $query = "SELECT DISTINCT nama_sa FROM target_serviceadvisor";
                                    $hasil = mysql_query($query);
                                    while ($data = mysql_fetch_array($hasil))
                                    {
									if($_POST[nama_sa]==$data[nama_sa]){
										echo "<option value='".$data['nama_sa']."' selected>".$data['nama_sa']."</option>";
									}
									else{
										echo "<option value='".$data['nama_sa']."'>".$data['nama_sa']."</option>";
									}
                                    }
                                    ?>
                                    </select>
                    ...
                    if(isset($_POST['cari'])){
                    	...
                    	$kueri="SELECT * FROM target_serviceadvisor";
						$bulan=$_POST['bulan'];
						$nama_sa=$_POST['nama_sa'];
						$target_unit=$_POST['target_unit'.$nomor];
						$target_point=$_POST['target_point'.$nomor];
						...
						if (empty($bulan)){
                    		...
                    		?>
                                ...
                                <div class="table-header blink"><font color="red"><i><b>Target Periode: <?php echo $_POST['bulan']?> dengan Service Advisor: <?php echo $_POST['nama_sa']?></b></i></font></div><br />
                                
                        <?php
                    		
                    		$query=mysql_query("$kueri where bulan='$bulan' and nama_sa='$nama_sa' order by urutan asc");
                        ...
                        <table class="table table-bordered table-hover" id="sample-table-1">
							...
							<input type="hidden" name="<?php echo 'nama_sa'.$nomor; ?>" value="<?php echo $nama_sa;?>" />
                            <?php
...
<?php 
	...
	if(count($_POST)) {
                        ...
                        // Cek username di database
                        $data=mysql_query("select * from target_serviceadvisor where bulan='$_POST[bulan]'");
					    $sql = $data;
						$nomor = 0;
						while($data = mysql_fetch_array($sql)){
						$nomor += 1; 
...
window.location = "media_showroom.php?module=service_master_target_service_target_sa";
</script>;
...
<?php
    ...
    $query	= mysql_query($sql);
		...
		header('location:../media_showroom.php?module=service_master_target_service_target_sa');
