...
?>
					...
					if(count($_POST['ubah'])) {
                        ...
                        $data=mysql_query("select * from target_poles_bp where bulan='$_POST[bulan]'");
					    $sql = $data;
						$number = 0;
						...
						$number += 1; 
								...
								$no_id = $_POST ['no_id'.$number];
								
                        // Kalau username valid, inputkan data ke tabel users
                        
                       mysql_unbuffered_query("update target_poles_bp set target_point='$target_point' where kode_item='$kode_item' and bulan='$bulan'");
					  ...
					  //nama_kategori='$nama_kategori', target_unit='$target_unit', target_target_point='$target_target_point' where id='$id' and bulan='$bulan'");
				...
				<div class="main-content" >
					<div class="wrap-content container" id="container">
						...
						<section id="page-title" class="padding-top-15 padding-bottom-15">
							<div class="row">
								<div class="col-sm-8">
									<h1 class="mainTitle">Data Target Poles BP</h1>
									<span class="mainDescription">Input Target Poles BP</span>
								</div>
								...
								<div class="col-md-12">
									<!--h5 class="over-title margin-bottom-15">Keseluruhan <span class="text-bold">Data Sales</span></h5-->
										...
										<p class="progress-demo">
											<button type = "submit" class="btn btn-wide btn-primary ladda-button" data-style="expand-right" onclick=window.location.href='?module=service_master_target_service_poles_bp&act=buat';>
												<span class="ladda-label"><i class="fa fa-plus"></i> Input Target Poles BP</span>
											</button>	
										</p>
										<hr>
										<?php
                                    ...
                                    <?php
                                    $query = "SELECT DISTINCT bulan FROM target_poles_bp";
                    ...
                    if(isset($_POST['cari'])){
                    	...
                    	$bulan=$_POST['bulan'];
						$kueri="SELECT * FROM target_poles_bp";
                    	if (empty($bulan)){
                            ...
                            ?>
                                <tr>
                                	<td align="center"><?php echo $no ?>.</td>
                                		<input type="hidden" name="<?php echo 'no_id'.$number; ?>" value="<?php echo $row['id'];?>" />
                            		<td align = 'center'>
										<input type="hidden" name="<?php echo 'grup'.$number; ?>" value="<?php echo $row['grup'];?>" />
                            			<?php echo $row['grup'];?>
                            		</td>                                	
                                	<td align = 'center'>
										<input type="hidden" name="<?php echo 'tipe'.$number; ?>" value="<?php echo $row['tipe'];?>" />
                            			<?php echo $row['tipe'];?>
                            		</td>                                	
                                	<td align = 'center'>
									...
									<td align='center'>
                                	    <input type="text" value="<?php echo $row['target_point'];?>" name="<?php echo 'target_point'.$number; ?>" id="target"></input>
                            		</td>
                                </tr>
...
<?php 
	break;
	case "buat":
	include "input_target_poles_bp.php";
...
<?php 
	...
	if(count($_POST)) {
						...
						$target_target_point = $_POST['target_target_point'.$number];
						$kode_item = $_POST['kode_item'.$number];
						$nama_item = $_POST['nama_item'.$number];
                      ...
                      mysql_unbuffered_query("update target_poles_bp set target_unit='$target_unit', target_target_point='$target_target_point' where id='$id' and bulan='$bulan'");
...
window.location = "media_showroom.php?module=service_master_target_service_poles_bp";
</script>;

<?php
    break;
    case "hapustarget":
    $sql 	= "delete from target_poles_bp where bulan ='$bulan'";
    $query	= mysql_query($sql);
   
		header('location:../media_showroom.php?module=service_master_target_service_poles_bp');
