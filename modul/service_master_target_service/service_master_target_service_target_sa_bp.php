...
?>
					...
					if(count($_POST['ubah'])) {
                        ...
                        // Cek username di database
                         $data=mysql_query("select * from target_serviceadvisor_bp where bulan='$_POST[bulan]' and nama_bp = '$_POST[nama_bp]' order by urutan asc ");
					    $sql = $data;
						$nomor = 0;
						...
						mysql_query("update target_serviceadvisor_bp set fix_pembagi='$fix_pembagi', nama_item='$nama_item', target_unit='$target_unit', target_point='$target_point' where kode_item='$kode_item' and nama_bp='$nama_bp' and bulan ='$bulan'");
				...
				<div class="main-content" >
					<div class="wrap-content container" id="container">
						...
						<section id="page-title" class="padding-top-15 padding-bottom-15">
							<div class="row">
								<div class="col-sm-8">
									<h1 class="mainTitle">Data Target SA BP</h1>
									<span class="mainDescription">Input Data Target Service Advisor BP</span>
								</div>
								...
								<div class="col-md-12">
									<!--h5 class="over-title margin-bottom-15">Keseluruhan <span class="text-bold">Data Sales</span></h5-->
										...
										<p class="progress-demo">
											<button type = "submit" class="btn btn-wide btn-primary ladda-button" data-style="expand-right" onclick=window.location.href='?module=service_master_target_service_target_sa_bp&act=buat';>
												<span class="ladda-label"><i class="fa fa-plus"></i> Input Target SA BP Perbulan</span>
											</button>	
										</p>
										<hr>
										<?php
                                    ...
                                    <?php
                                    $query = "SELECT DISTINCT bulan FROM target_serviceadvisor_bp";
									...
									<select name="nama_bp" id="nama_bp">
									<option value='' selected disabled class="blink">&#9654; PILIH SERVICE ADVISOR &#9664;</option>
                                    <?php
                                    $query = "SELECT DISTINCT nama_bp FROM target_serviceadvisor_bp";
                                    $hasil = mysql_query($query);
                                    while ($data = mysql_fetch_array($hasil))
                                    {
									if($_POST[nama_bp]==$data[nama_bp]){
										echo "<option value='".$data['nama_bp']."' selected>".$data['nama_bp']."</option>";
									}
									else{
										echo "<option value='".$data['nama_bp']."'>".$data['nama_bp']."</option>";
									}
                                    }
                                    ?>
                                    </select>
                    ...
                    if(isset($_POST['cari'])){
                    	...
                    	$kueri="SELECT * FROM target_serviceadvisor_bp";
						$bulan=$_POST['bulan'];
						$nama_bp=$_POST['nama_bp'];
						$target_unit=$_POST['target_unit'.$nomor];
						$target_point=$_POST['target_point'.$nomor];
						...
						if (empty($bulan)){
                    		...
                    		?>
                                ...
                                <div class="table-header blink"><font color="red"><i><b>Target Periode: <?php echo $_POST['bulan']?> dengan Service Advisor BP: <?php echo $_POST['nama_bp']?></b></i></font></div><br />
                                
                        <?php
                    		
                    		$query=mysql_query("$kueri where bulan='$bulan' and nama_bp='$nama_bp' order by urutan asc");
                        ...
                        <table class="table table-bordered table-hover" id="sample-table-1">
							...
							<input type="hidden" name="<?php echo 'nama_bp'.$nomor; ?>" value="<?php echo $nama_bp;?>" />
                            <?php
...
<?php 
	break;
	case "buat":
	include "input_target_sa_bp.php";
...
<?php 
	...
	if(count($_POST)) {
                        ...
                        // Cek username di database
                        $data=mysql_query("select * from target_serviceadvisor_bp where bulan='$_POST[bulan]'");
					    $sql = $data;
						$nomor = 0;
						while($data = mysql_fetch_array($sql)){
						$nomor += 1; 
...
window.location = "media_showroom.php?module=service_master_target_service_target_sa_bp";
</script>;
...
<?php
    ...
    $query	= mysql_query($sql);
		...
		header('location:../media_showroom.php?module=service_master_target_service_target_sa_bp');
