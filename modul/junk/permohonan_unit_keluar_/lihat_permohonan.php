<?php
	...
	$a = "select * from unit_keluar where md5(md5(no_spk)) = '$id'";
	...
	$tgl = date("Y-m-d H:i:s")	;	
		...
		if($_SESSION['leveluser']=='supervisor'){
			mysql_unbuffered_query("update unit_keluar set spv_app = '$_POST[status_approved]', spv_user_app = '$_SESSION[namalengkap]', spv_app_time='$app_time' where no_spk = '$id'");
		}elseif($_SESSION['leveluser']=='ar_finance'){
			if($r['spv_app']=='Y'){
				mysql_unbuffered_query("update unit_keluar set arfinance_app = '$_POST[status_approved]', arfinance_user_app = '$_SESSION[namalengkap]', arfinance_app_time='$app_time' where no_spk = '$id'");
			}else{
			}
			
		}elseif($_SESSION['leveluser']=='spv_finance'){
			if($r['arfinance_app']=='Y'){
				mysql_unbuffered_query("update unit_keluar set spv_finance_app = '$_POST[status_approved]', spv_finance_user_app = '$_SESSION[namalengkap]', spv_finance_app_time='$app_time' where no_spk = '$id'");
			}else{
			}
			
		}elseif($_SESSION['leveluser']=='mngr_finance'){
			if($r['spv_finance_app']=='Y'){
				mysql_unbuffered_query("update unit_keluar set mngr_finance_app = '$_POST[status_approved]', mngr_finance_user_app = '$_SESSION[namalengkap]', mngr_finance_app_time='$app_time' where no_spk = '$id'");
			}else{
			}
						...
						<section id="page-title">
							<div class="row">
								<div class="col-sm-8">
									<h1 class="mainTitle">
									<?php 
										if ($_GET[act] == 'lihatpermohonan'){echo "Lihat Permohonan Unit Keluar";}
