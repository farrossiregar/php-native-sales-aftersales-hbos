...
/*	if ($_GET[act] == 'approvepengajuan') {
	...
	if ($_GET[act] == 'approvepengajuan') {
		mysql_unbuffered_query("update pengajuan_discount set status_approved = '$_POST[status_approved]', proses_approve = 'Y',user_approve = '$_SESSION[namalengkap]',ket_approve = '$_POST[ket_approve]',tgl_approve = '$tgl' where no_pengajuan = '$_GET[id]'");
		mysql_unbuffered_query("update notif set read_drksi = 'Y' where no_pengajuan = '$_GET[id]'");
	}
	elseif 	($_GET[act] == 'ajukanapprove') {
		mysql_unbuffered_query("update pengajuan_discount set status_approved = '$_POST[status_approved]', proses_approve = 'Y',user_approve = '$_SESSION[namalengkap]',ket_approve = '$_POST[ket_approve]',ket_permohonan = '$_POST[ket_permohonan]' where no_pengajuan = '$_GET[id]'");
		mysql_unbuffered_query("update notif set notif_drksi = 'Y' where no_pengajuan = '$_GET[id]'");
	}
	...
	if ($_POST[status_approved] =="1"){
		...
		";
						...
						$email_pemohon = mysql_fetch_array(mysql_query(" SELECT pd.no_pengajuan,u.email FROM pengajuan_discount pd left join users u on u.username = pd.username_pemohon where pd.no_pengajuan = '$_GET[id]' ")) ;
	                    $email_pemohon = $email_pemohon['email'];
										...
										<div class="row">
											...
											<?php 
												$data2 = mysql_query("SELECT pd.*,t.nama_tipe,m.nama_model,w.nama_warna FROM pengajuan_discount_ulang  pd
																	...
																	left join warna w on w.kode_warna = pd.warna where pd.no_pengajuan = '$_GET[id]' order by nomor ");
											...
											<div class="panel panel-white collapses" id="panel5">
												...
												<div class="radio clip-radio radio-primary radio-inline">												
													...
													<label for="radio1">
														Setujui<?php echo $data_cek_approve['no_pengajuan']?>
													</label>
												</div>
												<?php }?>
												<div class="radio clip-radio radio-primary radio-inline">
												...
												<button type = "button" class="btn btn-wide btn-danger ladda-button" data-style="expand-left" onclick=window.location.href='media_showroom.php?module=sub_transaksi_pengajuan_discount';>
													<span class="ladda-label"><i class="fa fa-mail-reply"></i> Keluar </span>
												</button>
