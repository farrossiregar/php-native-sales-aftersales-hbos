...
if (count($_POST)){
	...
	$query = "select j.nama as nama_model,KC.*,GC.* from srvt_kendaraancustomer  KC
			  ...
			  left join SrvM_TipeService T on t.kode = kc.Kode_Tipe
			  left join UntM_Jenis J on j.kode = t.jenis
	...
	if ($row_count > 0){
		...
		while ($data = sqlsrv_fetch_array($result)){
			$data_ajax = array ('status'=>"OK",'norangka'=>$data['NoRangka'],'nomesin'=>$data['NoMesin'],'tahunbuat'=>$data['TahunPembuatan'],'tipe'=>$data['Tipe'],'warna'=>$data['Warna'],
								...
								'noktp'=>$data['noktp'],'nohp'=>$data['nohp'],'nohp2'=>$data['nohp2'],'namapic'=>$data['namapic'],'nohppic'=>$data['nohppic'],'agama'=>$data['agama'],
