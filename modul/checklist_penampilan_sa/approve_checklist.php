<?php
	...
	$a = "select * from pengecekan_penampilan_sa_detail where no_pengecekan_mingguan = '$_GET[id]'";
	...
	$tgl = date("Y-m-d H:i:s")	;	
		...
		if($_SESSION['leveluser']=='mngr_bengkel'){
			mysql_unbuffered_query("update pengecekan_penampilan_sa set sign_atasan2 = '$_POST[status_approved]', sign_atasan2_user = '$_SESSION[username]', sign_atasan2_time='$app_time' where no_pengecekan_mingguan = '$_GET[id]'");
		}elseif($_SESSION['leveluser']=='DRKSI'){
			mysql_unbuffered_query("update pengecekan_penampilan_sa set sign_atasan1 = '$_POST[status_approved]', sign_atasan1_user = '$_SESSION[username]', sign_atasan1_time='$app_time' where no_pengecekan_mingguan = '$_GET[id]'");
		}else{
		}
	...
	$pengecekan_detail = mysql_query("select * from pengecekan_penampilan_sa_detail where no_pengecekan_mingguan = '$_GET[id]'");
	$data_pengecekan_detail = mysql_fetch_array($pengecekan_detail);
	
	$pengecekan_sa_approve = mysql_query("select * from pengecekan_penampilan_sa where no_pengecekan_mingguan = '$_GET[id]'");
	$data_pengecekan_sa_approve  = mysql_fetch_array($pengecekan_sa_approve );
...
<script>
				function lihat_keterangan($id){
					...
					$.ajax({
						method : "post",
						url : "modul/checklist_penampilan_sa/lihat_keterangan.php",
						...
						<section id="page-title">
							<div class="row">
								<div class="col-sm-8">
									<h1 class="mainTitle">
									<?php 
										if ($_GET[act] == 'approve'){echo "Approve Pengecekan Penampilan SA BP";}
									//	elseif ($_GET[act] == 'ajukanapprove'){echo "Ajukan Pengajuan Discount";}


									?></h1>
									<!--span class="mainDescription">Melihat data seluruh sa, tambah sa dan hapus sa.</span-->
								</div>
								<ol class="breadcrumb">
									<li>
										<span>Checklist</span>
									...
									<li class="active">
										<span>Checklist Penampilan SA BP</span>
									</li>
								</ol>
							</div>
						</section>
										...
										<div class="row">
											...
											<div class="panel panel-white collapses" id="panel5">
													...
													<div class="panel-heading">
														<h4 class="panel-title text-primary">Detail Checklist Penampilan SA BP</h4>
													...
													<?php
														$pengecekan_showroom = mysql_query("select * from pengecekan_penampilan_sa where no_pengecekan_mingguan = '$data_pengecekan_detail[no_pengecekan_mingguan]'");
														...
														<div class="table-responsive">
															<table class="table table-bordered table-hover" id="sample-table-1">
																<tbody>
																	<tr class="info">
																		<?php
																			$detail_pengecekan = mysql_query("select tanggal from pengecekan_penampilan_sa_detail where no_pengecekan_mingguan = '$data_pengecekan_detail[no_pengecekan_mingguan]' group by tanggal");
																	...
																	<?php
																		$bulan = date('m-Y');
																		$sql=mysql_query("select * from sa_bp order by no");
																		$no1 = 0;
																		$no2 = 0;
																	...
																	<tr class="warning">
																		<td><b><?php echo strtoupper($data_sql[kode_sa_bp]) ?></b></td>
																		<td><b>Pukul</b></td>
																		<?php
																			$detail_pengecekan = mysql_query("select * from pengecekan_penampilan_sa_detail where kode_sa = '$data_sql[kode_sa_bp]' and no_pengecekan_mingguan = '$data_pengecekan_detail[no_pengecekan_mingguan]' group by tanggal");
																	...
																	<?php
																		$nama_pengecekan = mysql_query("select * from master_penilaian_sa");
																	...
																	<tr class="info">
																	<?php
																		$detail_pengecekan = mysql_query("select * from pengecekan_penampilan_sa_detail where kode_sa = '$data_sql[kode_sa_bp]' and jenis_penilaian = '$data_nama_pengecekan[jenis_penilaian]' and jam = '08:00' group by tanggal order by jenis_penilaian");
																			while($data_detail_pengecekan = mysql_fetch_array($detail_pengecekan)){
																	?>
																		<td><?php echo $data_detail_pengecekan[jenis_penilaian] ?></b></td>
																		<td><?php echo $data_detail_pengecekan[jam] ?></td>
																		<?php
																			$detail_pengecekan = mysql_query("select * from pengecekan_penampilan_sa_detail where kode_sa = '$data_sql[kode_sa_bp]' and jenis_penilaian = '$data_nama_pengecekan[jenis_penilaian]' and no_pengecekan_mingguan = '$data_pengecekan_detail[no_pengecekan_mingguan]' and jam = '08:00' group by tanggal asc");
																			while($data_detail_pengecekan = mysql_fetch_array($detail_pengecekan)){
																		?>
																		<td>
																			<?php 
																	...
																	<tr class="info">
																	<?php
																		$detail_pengecekan = mysql_query("select * from pengecekan_penampilan_sa_detail where kode_sa = '$data_sql[kode_sa_bp]' and jenis_penilaian = '$data_nama_pengecekan[jenis_penilaian]' and jam = '13:00' group by tanggal asc");
																			while($data_detail_pengecekan = mysql_fetch_array($detail_pengecekan)){
																	?>
																		<td><?php echo $data_detail_pengecekan[jenis_penilaian] ?></b></td>
																		<td><?php echo $data_detail_pengecekan[jam] ?></td>
																		<?php
																			$detail_pengecekan = mysql_query("select * from pengecekan_penampilan_sa_detail where kode_sa = '$data_sql[kode_sa_bp]' and jenis_penilaian = '$data_nama_pengecekan[jenis_penilaian]' and no_pengecekan_mingguan = '$data_pengecekan_detail[no_pengecekan_mingguan]' and jam = '13:00' group by tanggal asc");
																			while($data_detail_pengecekan = mysql_fetch_array($detail_pengecekan)){
																		?>
																		<td>
																			<?php 
															...
															<?php
																$detail_pengecekan = mysql_query("select * from pengecekan_penampilan_sa_detail where no_pengecekan_mingguan = '$data_pengecekan_detail[no_pengecekan_mingguan]' and hasil_penilaian = 'N'");
															...
															<table class="table table-bordered table-hover" id="sample-table-1">
																<tr class = 'info'>
																	<td colspan = '6'><b><?php echo strtoupper('DETAIL PENGECEKAN PENAMPILAN SA') ?></b></td>
																	
																</tr>
																<tr class = 'warning'>
																	<th ><b><?php echo strtoupper('NO') ?></b></th>
																	<th ><b><?php echo strtoupper('KODE SA BP') ?></b></th>
																...
																<?php
																	$detail_pengecekan = mysql_query("select * from pengecekan_penampilan_sa_detail where no_pengecekan_mingguan = '$data_pengecekan_detail[no_pengecekan_mingguan]' and hasil_penilaian = 'N' and catatan_pengecekan != '' group by tanggal, kode_sa, jam order by tanggal asc, jam asc");
																	$no = 0;
																	while($data_detail_pengecekan = mysql_fetch_array($detail_pengecekan)){
																		$no = $no+1;
																?>
																<tr class="info">
																	<td><?php echo $no ?></td>
																	<td>
																		<?php echo $data_detail_pengecekan['kode_sa'] ?>
																	</td>
																	<td>
												...
												<?php 
												if ($_GET[act] == 'approve'){ 
													if(($_SESSION['leveluser']=='mngr_bengkel' and $data_pengecekan_sa_approve ['sign_atasan2'] == '') or ($_SESSION['leveluser']=='DRKSI' and $data_pengecekan_sa_approve['sign_atasan1'] != 'Y' and $data_pengecekan_sa_approve['sign_atasan2'] == 'Y') or $_SESSION['leveluser'] == 'admin' ){ 
												?>
