...
//	$query = mysql_query("SELECT * FROM pemasangan_aksesoris where no_spk = 'JHGBUYG'"); 
	$query = mysql_query("SELECT * FROM pengecekan_penampilan_sales where no_pengecekan_mingguan = '$_GET[no_pengecekan_mingguan]'"); 
...
$html1 = "<table>
			<tr>
				<td width='500%'>
					<br>
					<h4><b>PENILAIAN PENAMPILAN SALES : </b></h4>
				</td>
			</tr>
...
$pdf->WriteHTML($html1);
$tgl_pengecekan = mysql_query("select tanggal from pengecekan_penampilan_sales_detail where no_pengecekan_mingguan = '$no_pengecekan_mingguan' group by tanggal");
...
$master_kategori = mysql_query("select * from target_sales where kode_spv = 'SUDI' and bulan = '$bln'");
$n = 0;
...
$pdf->cell(50,4.5,strtoupper($data_master_kategori['kode_sales']),0,1);
...
//	$pengecekan_showroom_detail = mysql_query("select * from pengecekan_showroom_detail where kategori_penilaian = '$data_master_kategori[kategori_penilaian]' and no_pengecekan_mingguan = '$no_pengecekan_mingguan' order by nama_penilaian, tanggal");
	$pengecekan_showroom_detail = mysql_query("select * from pengecekan_penampilan_sales_detail where kode_sales = '$data_master_kategori[kode_sales]' and no_pengecekan_mingguan = '$no_pengecekan_mingguan' group by jenis_penilaian");
	...
	//	$pdf->cell(23,5.5,$data_pengecekan_showroom_detail['hasil'],0,0);
			...
			$pengecekan_showroom_detail2 = mysql_query("select * from pengecekan_penampilan_sales_detail where jenis_penilaian = '$nama_penilaian' and no_pengecekan_mingguan = '$no_pengecekan_mingguan' and kode_sales = '$data_master_kategori[kode_sales]' order by tanggal, jam");
...
$pdf->WriteHTML($html3);
$keterangan = mysql_query("select * from pengecekan_penampilan_sales_detail where no_pengecekan_mingguan = '$_GET[no_pengecekan_mingguan]' and hasil_penilaian = 'N' order by kode_sales, jenis_penilaian, tanggal");
$no = 0;
while($data_keterangan = mysql_fetch_array($keterangan)){
	$no += 1;
	$pdf->cell(5,5.5,$no.'.',0,0);
	$pdf->cell(8,5.5,strtoupper($data_keterangan['kode_sales']).' ('.strtoupper($data_keterangan['jenis_penilaian']).') ('.$data_keterangan['tanggal'].' '.$data_keterangan['jam'].')',0,1);
	$pdf->cell(8,5.5,'',0,0);
