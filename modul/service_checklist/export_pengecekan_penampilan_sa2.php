...
?>
		...
		<table border="1">
			...
			<?php
				$master = mysql_query("SELECT * from sa_bp");	
				$no = 0;
			...
			?>
				...
				<tr>
					...
					<td border = '1' rowspan = "2" valign = "middle"><?php echo $data_master['kode_sa_bp']; ?></td>
					<td border = '1'>08:00</td>
					<td></td>
					<?php
						$tanggal_pengecekan = mysql_query("select tanggal from pengecekan_penampilan_sa_detail where substr(tanggal, 6, 2) = '$_GET[bulan]' group by tanggal");
						while($data_tanggal_pengecekan = mysql_fetch_array($tanggal_pengecekan)){
							$tgl_cek = substr($data_tanggal_pengecekan['tanggal'], 8, 2);
							$penilaian = mysql_query("SELECT hasil_penilaian, catatan_pengecekan from pengecekan_penampilan_sa_detail where jam = '08:00' and kode_sa = '$data_master[kode_sa_bp]' and substr(tanggal, 9, 2) = '$tgl_cek' group by kode_sa");	
							$no = 0;
				...
				<tr>
					<td border = '1'>13:00</td>
					<td></td>
					<?php
						$tanggal_pengecekan = mysql_query("select tanggal from pengecekan_penampilan_sa_detail where substr(tanggal, 6, 2) = '$_GET[bulan]' group by tanggal");
						while($data_tanggal_pengecekan = mysql_fetch_array($tanggal_pengecekan)){
							$tgl_cek = substr($data_tanggal_pengecekan['tanggal'], 8, 2);
							$penilaian = mysql_query("SELECT hasil_penilaian, catatan_pengecekan from pengecekan_penampilan_sa_detail where jam = '13:00' and kode_sa = '$data_master[kode_sa_bp]' and substr(tanggal, 9, 2) = '$tgl_cek' group by kode_sa");	
							$no = 0;
							while($data_penilaian = mysql_fetch_array($penilaian)){
								$no = $no+1;
								if($data_penilaian['hasil_penilaian'] == "Y"){
			...
			<tr>
				...
				<td>
					<table border="1">
						...
						<?php
							$keterangan_pengecekan = mysql_query("select * from pengecekan_penampilan_sa_detail where substr(tanggal, 6, 2) = '$_GET[bulan]' and hasil_penilaian = 'N'");
						...
						?>
							...
							<?php
								$keterangan_pengecekan = mysql_query("select * from pengecekan_penampilan_sa_detail where substr(tanggal, 6, 2) = '$_GET[bulan]' and jenis_penilaian = '$nama_pengecekan' and hasil_penilaian = 'N'");
								while($data_keterangan_pengecekan = mysql_fetch_array($keterangan_pengecekan)){
									$nama_pengecekan = $data_keterangan_pengecekan['jenis_penilaian'];
									$tgl_pengecekan = $data_keterangan_pengecekan['tanggal'];
									$catatan_pengecekan = $data_keterangan_pengecekan['catatan_pengecekan'];
