				...
				<div class="main-content" >
					<div class="wrap-content container" id="container">
						...
						<div class="container-fluid container-fullw bg-white">
							...
							$filter_tanggal = ($status_approved == "" ? "input_form" : ($status_approved == "ADM_APP" ? "salesadm_app_time" : ($status_approved == "MNGR_APP" ? "mngr_app_time" : ($status_approved == "SPV_APP" ? "spv_app_time" : ($status_approved == "SEMUA" ? "input_form" : ($status_approved == "N" ? "input_form" : ($status_approved == "FIN_APP" ? "keuangan_app_time" : "input_form")))))));
							...
							if (empty($tgl_awal) and empty($tgl_akhir)){
								...
								*/
								...
								{
									$query=mysql_query("$kueri where $filter_approved $filter_pemesanan $filter_pasang substr(date($filter_tanggal), 1, 10) >='$_GET[tgl_awal]' and substr(date($filter_tanggal), 1, 10) <='$_GET[tgl_akhir]' and kode_spv = '$_SESSION[kode_spv]' order by pa.no_permohonan desc");
								}
								elseif ($_SESSION['leveluser'] == 'user') 
								{
									$query=mysql_query("$kueri where $filter_approved $filter_pemesanan $filter_pasang substr(date($filter_tanggal), 1, 10) >='$_GET[tgl_awal]' and substr(date($filter_tanggal), 1, 10) <='$_GET[tgl_akhir]' and user_input = '$_SESSION[username]' order by pa.no_permohonan desc");
								}
								elseif ($_SESSION['leveluser'] != 'supervisor' and $_SESSION['leveluser'] != 'user')
								{
									$query=mysql_query("$kueri where $filter_approved $filter_pemesanan $filter_pasang substr(date($filter_tanggal), 1, 10) >='$_GET[tgl_awal]' and substr(date($filter_tanggal), 1, 10) <='$_GET[tgl_akhir]' order by pa.no_permohonan desc");
								}
                            ...
                            <?php
								...
								while($row=mysql_fetch_assoc($query)){
									$id = substr(md5(md5(addslashes($row['no_permohonan']))),0,28).md5($row['no_permohonan']);
									
									
									$tombol="<a class='btn btn-xs btn-success' href='media_showroom.php?module=aksesoris_pemasangan_aksesoris&act=approvedpemasangan&id=$id' 
									...
									$tombol_status_pemesanan="<a class='btn btn-xs btn-dark-beige' href='media_showroom.php?module=aksesoris_pemasangan_aksesoris&act=ubahstatuspemesanan&id=$id' 
									...
									$tombol_status_pemasangan="<a class='btn btn-xs btn-dark-beige' href='media_showroom.php?module=aksesoris_pemasangan_aksesoris&act=ubahstatuspemasangan&id=$id' 
								...
								?>
									<tr>
										...
										<td><?php echo '<strong>No Form : </strong>'. $row['no_permohonan'] .' / '.$row['input_form'].'<br />
												...
												if($row['spv_app']=='Y'){
													echo "</br><b>Disetujui oleh Supervisor : </b>".strtoupper($row['spv_user_app'])." / ".$row['spv_app_time']."</br>";
												}elseif($row['spv_app']=='N'){
													echo "</br><b style='color:red'>Tidak Disetujui oleh Supervisor : </b>".strtoupper($row['spv_user_app'])."</br>";
												}	
											
												if($row['mngr_app']=='Y'){
													echo "<b>Disetujui oleh Manager : </b>".strtoupper($row['mngr_user_app'])." / ".$row['mngr_app_time']."</br>";
											...
											?>
														...
														<a id="tambahlagi" class='btn btn-xs btn-info' href='media_showroom.php?module=aksesoris_pemasangan_aksesoris&act=tambahlagi&id=<?php echo substr(md5(md5(addslashes($row['no_permohonan']))),0,28).md5($row['no_permohonan']); ?>' data-placement='top' data-toggle='tooltip' data-original-title='Tambah Item Aksesoris Nomor Spk : <?php echo $row['no_spk'] ?>'><i class='fa fa-check'></i> Tambahan Aksesoris</a>
											<?php			
													}
											...
											<a class='btn btn-xs btn-info' href='media_showroom.php?module=aksesoris_pemasangan_aksesoris&act=lihat&id=<?php echo substr(md5(md5(addslashes($row['no_permohonan']))),0,28).md5($row['no_permohonan']); ?>' data-placement='top' data-toggle='tooltip' data-original-title='Lihat Data No Permohonan : <?php echo $row['no_permohonan'] ?>'><i class='fa fa-check'></i> Lihat Data</a>
											<?php			
...
<?php 
	break;
	case "buat":
	include "action/pemasangan_aksesoris_buat.php";
		

	break;
	case "tambahlagi":
	include "action/pemasangan_aksesoris_tambah_lagi.php";
		

	break;
	case "lihat":
	include "action/pemasangan_aksesoris_approve.php";
	...
	break;
	case "update":
	include "action/pemasangan_aksesoris_ubah_data.php";
	break;
} 
}?>