<?php
	...
	$no_permohonan = addslashes($_GET['id']);
	//$no_permohonan = substr(md5(md5(addslashes($_GET['id']))),0,17).uniqid();
	
	$a = "select * from pemasangan_aksesoris where no_permohonan = '$no_permohonan'";
		...
		if ($_GET[act] == 'ubahstatuspemasangan'){
				...
				if($_SESSION['leveluser']=='staff_logistik'){
						...
						if ($status_pemasangan_akhir == "P"){
							mysql_unbuffered_query("update notif_pemasangan_aksesoris set read_logistik = 'N' where no_permohonan = '$r[no_permohonan]'");
						}
	...
	$a = mysql_query("select * from pemasangan_aksesoris where no_permohonan = '$no_permohonan'");
