		...
		<table border="1">
			<thead>
				...
				<tr >
					...
					<th width="3"></th>
					<?php
						$jumlah_data = mysql_query("select tanggal from pengecekan_showroom_detail where substr(tanggal, 6, 2) = '$_GET[bulan]' group by tanggal");
				...
				<tr >
					...
					<?php
						$tanggal_pengecekan = mysql_query("select tanggal from pengecekan_showroom_detail where substr(tanggal, 6, 2) = '$_GET[bulan]' group by tanggal");
			...
			?>
				...
				<tr>
					...
					<?php
						...
						while($data_tanggal_pengecekan = mysql_fetch_array($tanggal_pengecekan)){
							...
							$penilaian = mysql_query("SELECT hasil from pengecekan_showroom_detail where jam = '09:00' and nama_penilaian = '$data_master[nama_penilaian]' and substr(tanggal, 9, 2) = '$tgl_cek'");	
							$no = 0;
				...
				<tr>
					...
					<?php
						...
						while($data_tanggal_pengecekan = mysql_fetch_array($tanggal_pengecekan)){
							$tgl_cek = substr($data_tanggal_pengecekan['tanggal'], 8, 2);
							$penilaian = mysql_query("SELECT hasil from pengecekan_showroom_detail where jam = '14:00' and nama_penilaian = '$data_master[nama_penilaian]' and substr(tanggal, 9, 2) = '$tgl_cek'");	
							$no = 0;
							while($data_penilaian = mysql_fetch_array($penilaian)){
								$no = $no+1;
								if($data_penilaian['hasil'] == "Y"){
			...
			<tr>
				...
				<td>
					<table border="1">
						...
						<?php
							$keterangan_pengecekan = mysql_query("select * from pengecekan_showroom_detail where substr(tanggal, 6, 2) = '$_GET[bulan]' and hasil = 'N'");
