...
$pdf->WriteHTML($html1);
$tgl_pengecekan = mysql_query("select tanggal from pengecekan_showroom_detail where no_pengecekan_mingguan = '$no_pengecekan_mingguan' group by tanggal");
		$pdf->cell(88,5.5,'',0,0);
...
$pdf->cell(60,5.5,strtoupper($data_master_kategori['kategori_penilaian']),0,1);
...
//	$pengecekan_showroom_detail = mysql_query("select * from pengecekan_showroom_detail where kategori_penilaian = '$data_master_kategori[kategori_penilaian]' and no_pengecekan_mingguan = '$no_pengecekan_mingguan' order by nama_penilaian, tanggal");
	$pengecekan_showroom_detail = mysql_query("select * from pengecekan_showroom_detail where kategori_penilaian = '$data_master_kategori[kategori_penilaian]' and no_pengecekan_mingguan = '$no_pengecekan_mingguan' group by nama_penilaian,jam");
	...
	//	$pdf->cell(15,5.5,$tgl,1,0);
		$pdf->cell(60,5.5,$nama_penilaian,0,0);
		$pdf->cell(20,5.5,$jam,0,0);
	//	$pdf->cell(23,5.5,$data_pengecekan_showroom_detail['hasil'],0,0);
		
			$pengecekan_showroom_detail2 = mysql_query("select * from pengecekan_showroom_detail where nama_penilaian = '$nama_penilaian' and jam = '$jam' and no_pengecekan_mingguan = '$no_pengecekan_mingguan' and jam = '$data_pengecekan_showroom_detail[jam]' group by tanggal order by tanggal");
			...
			while($data_pengecekan_showroom_detail2 = mysql_fetch_array($pengecekan_showroom_detail2)){
				$pdf->cell(10, 5,$data_pengecekan_showroom_detail2['hasil'],1,0);
			}
			$total2 = 6;
			$res2 = $total2 - $total;
			$i;
...
$pdf->WriteHTML($html3);
$keterangan = mysql_query("select * from pengecekan_showroom_detail where no_pengecekan_mingguan = '$_GET[no_pengecekan_mingguan]' and hasil = 'N' order by tanggal, kategori_penilaian");
$no = 0;
while($data_keterangan = mysql_fetch_array($keterangan)){
	...
	$pdf->cell(8,5.5,'',0,0);
	$pdf->cell(8,5.5,"Catatan Pengecekan : ".strtolower($data_keterangan['catatan_pengecekan']).' ('.$data_keterangan['tanggal'].' '.$data_keterangan['jam'].')',0,1);
	$pdf->cell(8,5.5,'',0,0);
