<?php
	...
	$no_pengajuan = mysql_real_escape_string($_GET['id']);
	...
	$a = "select t.nama_tipe as nama_tipe, w.nama_warna as nama_warna, m.nama_model as nama_model ,pd.* from pengajuan_discount pd 
			...
			where md5(md5(pd.no_pengajuan)) = '$no_pengajuan'";
...
/*	if ($_GET[act] == 'approvepengajuan') {
		mysql_unbuffered_query("update pengajuan_discount set status_approved = '$_POST[status_approved]', proses_approve = 'Y',user_approve = '$_SESSION[namalengkap]',ket_approve = '$_POST[ket_approve]',tgl_approve = '$tgl' where no_pengajuan = '$_GET[id]'");
	}
	...
	if ($_GET[act] == 'approvepengajuan') {
		mysql_unbuffered_query("update pengajuan_discount set status_approved = '$_POST[status_approved]', proses_approve = 'Y',user_approve = '$_SESSION[namalengkap]',ket_approve = '$_POST[ket_approve]',tgl_approve = '$tgl' where no_pengajuan = '$r[no_pengajuan]'");
		mysql_unbuffered_query("update notif set read_drksi = 'Y' where no_pengajuan = '$r[no_pengajuan]'");
	}
	elseif 	($_GET[act] == 'ajukanapprove') {
		mysql_unbuffered_query("update pengajuan_discount set status_approved = '$_POST[status_approved]', proses_approve = 'Y',user_approve = '$_SESSION[namalengkap]',ket_approve = '$_POST[ket_approve]',ket_permohonan = '$_POST[ket_permohonan]' where no_pengajuan = '$r[no_pengajuan]'");
		mysql_unbuffered_query("update notif set notif_drksi = 'Y' where no_pengajuan = '$r[no_pengajuan]'");
	}
	...
	if ($_POST[status_approved] =="1"){
		...
		";
                        ...
                        $a = "select t.nama_tipe as nama_tipe, w.nama_warna as nama_warna, m.nama_model as nama_model ,pd.* from pengajuan_discount pd 
                    			...
                    			left join model m on m.kode_model=pd.model
                    			left join warna w on w.kode_warna=pd.warna
                    			where pd.no_pengajuan = '$r[no_pengajuan]'";
						...
						$b = mysql_query("SELECT pd.*,t.nama_tipe,m.nama_model,w.nama_warna FROM pengajuan_discount_ulang  pd
										...
										left join warna w on w.kode_warna = pd.warna where pd.no_pengajuan = '$r[no_pengajuan]' ");
						...
						$email_pemohon = mysql_fetch_array(mysql_query(" SELECT pd.no_pengajuan,u.email FROM pengajuan_discount pd left join users u on u.username = pd.username_pemohon where no_pengajuan = '$r[no_pengajuan]' ")) ;
	                    $email_pemohon = $email_pemohon['email'];
    ...
    $dt = "select * from data_mobil where kode_tipe = '$r[tipe_mobil]' and kode_warna = '$r[warna]' and nomatching = '' and tahun_buat = $r[tahun_buat] ";
								...
								<script>
									...
									<form role="form" id="form" name="form" enctype="multipart/form-data" method="post" action="" >
										<div class="row">
													...
													?>
															...
															<table class="table table-bordered table-hover" id="sample-table-1">
																<tbody>
																	...
																	<tr class="info">
																		<td>Nama Sales</td>
																		<td><?php echo $r[nama_sales]." / ".$r[kode_spv] ?></td>
																	</tr>
																	<tr class="warning">
																		<td>Nama Customer</td>
																	...
																	<tr class="info">
																		<td>Model / Tahun</td>
																		<td><?php echo $model.' / '.$r['tahun_buat'] ?> </td>
																	</tr>
																	<tr class="warning">
																		<td>Tipe Mobil</td>
																	...
																	<tr class="info">
																		<td>Asal Prospek</td>
																		<td><?php echo $r[asal_prospek] . ($r[ket_asal_prospek] != "" ? " - $r[ket_asal_prospek]" : "" ) ?></td>
																	</tr>
																	<tr class="success">
																	...
																	<tr class="warning">
																		<td>Asuransi</td>
																		<td><?php echo ($r['cara_beli'] == 'KREDIT' ? "ASURANSI LEASING" : ($r[ikut_asuransi] == "Y" ? "IKUT ASURANSI" : ($r[ikut_asuransi] == "" ? "" : "TIDAK ASURANSI ($r[ket_asuransi])")).($r[ikut_asuransi] == "Y" ? " - $r[asuransi]" : "")) ; ?></td>
																	</tr>
																	<?php if($r[cara_beli] == "KREDIT"){ ?>
																	<tr class="info" >
																		<td>Leasing</td>
											...
											<?php 
												$data2 = mysql_query("SELECT pd.*,t.nama_tipe,m.nama_model,w.nama_warna FROM pengajuan_discount_ulang  pd
																	left join tipe t on t.kode_tipe = pd.tipe_mobil
																	left join model m on pd.model = m.kode_model
																	left join warna w on w.kode_warna = pd.warna where md5(md5(pd.no_pengajuan)) = '$_GET[id]' order by nomor ");
												...
												$model_ulang = $tampil[nama_model];
												$tipe_ulang = $tampil[nama_tipe];
												$warna_ulang = $tampil[nama_warna];
												
												
												
												$dt = "select * from data_mobil where kode_tipe = '$tampil[tipe_mobil]' and kode_warna = '$tampil[warna]' and nomatching = '' and tahun_buat = '$tampil[tahun_buat]' ";
												$cueri = mysql_query($dt);
												$sisa_stock = mysql_num_rows($cueri);
											...
											<div class="panel panel-white collapses" id="panel5">
															...
															<table class="table table-bordered table-hover" id="sample-table-1">
																<tbody>
																	...
																	<tr class="warning">
																		...
																		<tr class="info">
																		<td>Model / Tahun</td>
																		<td><?php echo $model_ulang.' / '.$tampil['tahun_buat'] ?> </td>
																	</tr>
																	<tr class="warning">
																		<td>Tipe Mobil</td>
																		<td> <?php echo $tampil[tipe_mobil] ." - $tipe_ulang" ?></td>
																	</tr>
																	<tr class="info">
																		<td>Warna Mobil</td>
																		<td> <?php echo $warna_ulang ?></td>
																	</tr>
																	<tr class="warning">
																		<td>Harga OTR</td>
																	...
																	<tr class="warning">
																		<td>Total Diskon Netto</td>
																		<td><?php echo ($disc_netto1 < 1 ? "Rp 0" : $disc_netto ) ?></td>
																	</tr>
																	<tr class="info">
																		<td>Metode Pembayaran</td>
																	...
																	<tr class="warning">
																		<td>Asuransi</td>
																		<td><?php echo ($tampil['cara_beli'] == "KREDIT" ? "ASURANSI LEASING" :($tampil['cara_beli'] == 'KREDIT' ? "ASURANSI LEASING" : ($tampil[ikut_asuransi] == "Y" ? "IKUT ASURANSI" : "TIDAK ASURANSI ($tampil[ket_asuransi])").($tampil[ikut_asuransi] == "Y" ? " - $tampil[asuransi]" : ""))) ; ?></td>
																	</tr>
																	<?php if($tampil[cara_beli] == "KREDIT"){ ?>
																	<tr class="info">
																		<td>Leasing</td>
												...
												if($_GET['notif']!='Y' and $_GET['info'] != 'Y'){
													?>
												<button type = "button" class="btn btn-wide btn-danger ladda-button" data-style="expand-left" onclick=<?php if(count($_POST)) { echo "history.go(-2)"; } else {echo "history.go(-1)";}?>>
													<span class="ladda-label"><i class="fa fa-mail-reply"></i> Keluar </span>
												</button>
												...
												<button type = "button" class="btn btn-wide btn-danger ladda-button" data-style="expand-left" onclick=window.location.href='media_showroom.php?module=prospek_pengajuandiscount';>
													<span class="ladda-label"><i class="fa fa-mail-reply"></i> Keluar </span>
												</button>
