...
date_default_timezone_set('Asia/Jakarta');
	...
	$kueri = mysql_query("select * FROM pengajuan_discount_ulang where md5(md5(no_pengajuan))='$_GET[id]'");
	if(mysql_num_rows($kueri)>=1){
		$double = 'Y';
		while($er = mysql_fetch_array($kueri)){
		$nama_customer_ulang=$er['nama_customer'];
    ...
    $sql = mysql_query("select * FROM pengajuan_discount where md5(md5(no_pengajuan))='$_GET[id]'");
	...
	if($double=='Y'){
		$nama_customer=$nama_customer_ulang;
	...
	$disc_approved = "Rp ".number_format("$r[disc_approved]",0,".",".");
        ...
        $len = strlen($ket_discount);
            if ($len > 61){
                ...
                $ket_discount4=substr($ket_discount,183,61);
                $ket_discount5=substr($ket_discount,244,61);
                $ket_discount6=substr($ket_discount,305,61);
                $ket_discount7=substr($ket_discount,366,61);
...
// MULAI JOIN NAMA, TIPE DAN WARNA MODEL
	...
	// TUTUP JOIN NAMA, TIPE DAN WARNA MODEL
		...
		if ($double =='Y' ){
		    ...
		    $query_pdu1 = "select t.nama_tipe as nama_tipe, w.nama_warna as nama_warna, m.nama_model as nama_model ,pd.* from pengajuan_discount_ulang pd 
		    left join tipe t on t.kode_tipe=pd.tipe_mobil 
		    left join model m on m.kode_model=pd.model left join warna w on w.kode_warna=pd.warna where md5(md5(pd.no_pengajuan))='$_GET[id]' ";
		    
		    $query_pdu = mysql_query($query_pdu1);
		    
		    while ($data_pdu = mysql_fetch_array($query_pdu))
		    {
		    
    		    $kode_tipe = trim($data_pdu['tipe_mobil']);
    		    $model = $data_pdu['nama_model'];
            	$tipe_mobil = $data_pdu['nama_tipe'];
                $warna = $data_pdu['nama_warna'];
                $harga_otr = "Rp ".number_format("$data_pdu[harga_otr]",0,".",".");
            	$discount_unit = "Rp ".number_format("$data_pdu[discount_unit]",0,".",".");
		    }
		}
...
<table border='0' cellspacing='10' cellpadding='10'>
  ...
  <tr>
    <td align='left'>&nbsp;</td>
    <td align='center' width='1'>&nbsp;</td>
    <td align='left' width='500'>$ket_discount4</td>
  </tr>
  <tr>
    <td align='left'>&nbsp;</td>
    <td align='center' width='1'>&nbsp;</td>
    <td align='left' width='500'>$ket_discount5</td>
  </tr>
  <tr>
    <td align='left'>&nbsp;</td>
    <td align='center' width='1'>&nbsp;</td>
    <td align='left' width='500'>$ket_discount6</td>
  </tr>
   <tr>
    <td align='left'>&nbsp;</td>
    <td align='center' width='1'>&nbsp;</td>
    <td align='left' width='500'>$ket_discount7</td>
  </tr>
  <tr>
