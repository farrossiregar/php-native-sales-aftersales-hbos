	...
	<?php 
		...
		date_default_timezone_set('Asia/Jakarta');
						...
						$query_cek = mysql_query("select * from pengajuan_discount where nama_customer = '$nama_customer' and username_pemohon = '$_SESSION[username]' and warna = '$warna' and tipe_mobil = '$tipe_mobil' ");
					...
					if ( strlen(trim($_POST['nama_sales'])) >= 2 AND strlen(trim($_POST['nama_customer'])) >= 2 AND strlen(trim($_POST['jenis_identitas'])) >= 1 AND strlen(trim($_POST['no_identitas'])) >= 2 AND strlen(trim($_POST['hp_customer'])) >= 10
						...
						if ($cek_double >= 1){
							header("location:../../../media_showroom.php?module=prospek_pengajuandiscount&status=gagal"); 
						}else {
                            $double = 'N';
							$postok = 'Y';
                        ...
                        mysql_unbuffered_query("insert into pengajuan_discount (no_pengajuan,hp_customer,alamat_customer,jenis_identitas,no_identitas,nama_customer,model,tipe_mobil,warna,harga_otr,discount_unit,pengajuan_disc,ket_discount,total_discount_accs,refund,total_discount,waktu
							...
							values('PD$nextNoTransaksi','$hp_customer','$alamat_customer','$jenis_identitas','$no_identitas','$nama_customer','$model','$tipe_mobil','$warna','$harga_otr','$discount_unit','$pengajuan_disc','$ket_discount','$total_discount_accs','$refund','$total_discount','$tgl_pengajuan','N','$cara_beli'
							,'$leasing','$tenor','$_SESSION[namalengkap]','$_SESSION[username]','$nama_sales','$_SESSION[kode_spv]','$tahun_buat','$promo_dealer','$tgl_pengajuan','$asal_prospek','$ket_asal_prospek','$ikut_asuransi','$asuransi','$ket_asuransi')");
						...
						header("location:../../../media_showroom.php?module=prospek_pengajuandiscount&status=ok"); 
						}
