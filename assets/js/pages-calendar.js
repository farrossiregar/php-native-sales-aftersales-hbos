var Calendar = function() {"use strict";
	...
	var setFullCalendarEvents = function() {
		...
		var d1 = ["22", "25"];
		var tostring1 = d1.toString();
		var trim1 = tostring1.trim();
		var split1 = trim1.split(",");
		var m1 = 3;
		var y1 = 2018;
		var titles = ["Dating With Girlfriend", ,"Propose", "Married"];
		var tostring = titles.toString();
		...
		//	content: 'Out to design conference',
			...
			title: 'Dating With Girlfriend',
		//	start: new Date(y1, m1, d1 , 12, 0),
		//	start: new Date(2018, 3, 25),
			start: new Date([2018, 3, 25], [2018, 3, 27]),
		//	end: new Date(y1, m1, d1 , 16, 30),
			className: 'event-generic',
			category: 'generic',
			allDay: true
		}, {
	...
	var runFullCalendar = function() {
		...
		//	$('.events-modal').modal();
			$('#modal_test_drive').modal();
		});
		$('.events-modal').on('hide.bs.modal', function(event) {
	//	$('#modal_test_drive').on('hide.bs.modal', function(event) {
			$(".form-full-event #event-id").val("");
	...
	var runFullCalendarValidation = function(el) {
		...
		formEvent.validate({
			...
			//	newEvent.start = new Date($('.form-full-event #start-date-time').val());
				newEvent.start = new Date($('.form-full-event #tgl_test_drive').val());
